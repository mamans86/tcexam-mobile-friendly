...
if (isset($test_id) and ($test_id > 0)) {
    ...
    if ($r = F_db_query($sql, $db)) {
        while ($m = F_db_fetch_array($r)) {
            ...
            $subjlist .= ' <a href="'.$_SERVER['SCRIPT_NAME'].'?link_action=deletesubject&amp;test_id='.$test_id.'&amp;tsubset_id='.$m['tsubset_id'].'" title="'.$l['h_delete'].'" class="deletebutton"><i class="fas fa-times"></i> '.$l['w_delete'].'</a>';
